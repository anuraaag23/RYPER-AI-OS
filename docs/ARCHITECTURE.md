...
> **Amendments (read this first):** this document is preserved as the
> original Phase 1 design record and is **not** rewritten as later
> phases evolved — see [`docs/PROJECT_STATE.md`](PROJECT_STATE.md) for
> what is actually built today, and [`docs/adr/`](adr/) for every place
> a later phase deliberately deviated from what follows, with the
> reasoning recorded. The most significant deviation:
> [`docs/adr/0014`](adr/0014-electron-desktop-shell.md) — §20's
> "native shells over Electron / Rust Core" plan was not what got
> built; every phase since actually built Core in TypeScript (§4.1
> below specifies Rust) and the Phase 12 desktop shell is Electron,
> hosting that TypeScript Core in-process. `platform/desktop/{windows,
macos,linux}` still contain this document's originally-planned native
> scaffolds, untouched, in case a future phase revisits that path.
