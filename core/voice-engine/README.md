...
- **`AudioDeviceSource`/`AudioCaptureSource`/`AudioPlaybackSink`**: a
  ...
  directly.
- **`WakeWordProvider`**: real offline keyword-spotting model integration
  (Porcupine-style or a platform's native model) is supplied by whoever
  wires up the provider; `WakeWordEngine`'s sensitivity/cooldown/multi-provider
