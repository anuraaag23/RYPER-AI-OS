...
export class CapabilityPermissions {
  ...
  async requestPermission(
    ...
    const requiredCapability =
      (operation && descriptor?.operationCapabilities?.[operation]) ??
    ...
    this.record(domain, actorId, requiredCapability, granted);
    return granted;
  }

  checkPermission(domain: CapabilityDomain, actorId: string, operation?: string): boolean {
    const descriptor = this.registry.get(domain);
    const requiredCapability =
      (operation && descriptor?.operationCapabilities?.[operation]) ??
      descriptor?.requiredCapability;
    if (!requiredCapability) return true;
    return this.broker.hasGrant(actorId, requiredCapability);
  }
  ...
  availablePermissions(actorId: string): readonly Capability[] {
    ...
    for (const descriptor of this.registry.list()) {
      ...
      if (descriptor.operationCapabilities) {
        for (const cap of Object.values(descriptor.operationCapabilities)) {
          if (this.broker.hasGrant(actorId, cap)) {
