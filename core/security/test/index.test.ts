...
describe("CapabilityBroker", () => {
  ...
  it("grant() pre-authorizes a capability directly for an actor", () => {
    const broker = new CapabilityBroker(() => false);
    broker.grant("ai-orchestrator", "notifications");
    expect(broker.hasGrant("ai-orchestrator", "notifications")).toBe(true);
    expect(() => broker.assertGranted("ai-orchestrator", "notifications")).not.toThrow();
    const log = broker.getAuditLog();
    expect(log.some((e) => e.actor === "ai-orchestrator" && e.action === "grant_capability")).toBe(true);
  });
  ...
  it("classifies every existing capability as safe or protected (Phase 13.9 lock-state prep — not enforcement)", () => {
    expect(getCapabilitySensitivity("microphone")).toBe("protected");
    expect(getCapabilitySensitivity("camera")).toBe("protected");
    expect(getCapabilitySensitivity("filesystem.write")).toBe("protected");
    expect(getCapabilitySensitivity("notifications")).toBe("safe");
    // Every capability the type currently allows has a classification —
    // none silently falls through to `undefined`.
    for (const capability of Object.keys(CAPABILITY_SENSITIVITY) as Array<
      keyof typeof CAPABILITY_SENSITIVITY
    >) {
      expect(["safe", "protected"]).toContain(CAPABILITY_SENSITIVITY[capability]);
    }
  });
});
