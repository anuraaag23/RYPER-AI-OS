...
`diagnostics`, `performance_monitoring`. See `capability-descriptors.ts`
for the full `CapabilityDescriptor` list, and `windows-adapter.ts`'s
`buildDispatchTable()` for every `(domain, operation)` pair each one
implements.
...
`automation.execute`); most domains are left ungated at the
`CapabilityBroker` level and rely on this package's own
...
- **`requiredCapability` coverage is partial**, as noted above — most
  ...
  is no `"audio"` or `"window_management"` capability).
