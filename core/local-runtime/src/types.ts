...
export type RuntimeKind = "llama-cpp" | "ollama" | "onnx" | "mlx" | "whisper-cpp" | "piper";
...
export interface ASRRequest {
  ...
  /** BCP-47 language hint, where the provider supports one (e.g. Whisper's `-l`). Omitted means auto-detect/provider default. */
  readonly language?: string;
  /** Forwarded from `InferenceContext.signal` by `LocalRuntimeManager` — providers that can cancel a real, in-flight process should honor this directly, not just rely on queue-level cancellation of not-yet-started work. */
...
export interface TTSRequest {
  ...
  /** Forwarded from `InferenceContext.signal` — see `ASRRequest.signal`. */
  readonly signal?: AbortSignal;
}
